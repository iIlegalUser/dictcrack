...
namespace DictCrack
{
    internal static class Cli
    {
        ...
        {
            if (args.Length < 1) { Console.Error.WriteLine("用法: dictcrack info <archive>"); return 2; }
            string path = args[0];
        ...
        private static CrackConfig ParseCrackArgs(string[] args)
        {
            ...
            cfg.ArchivePath = GetOpt(args, new string[] { "-a", "--archive" });
            List<string> dicts = new List<string>();
            for (int i = 0; i < args.Length; i++)
            {
                string a = args[i];
                string v;
                if (TryOpt(args, ref i, new string[] { "-w", "--dict" }, out v)) dicts.Add(v);
                else if (TryOpt(args, ref i, new string[] { "-w2" }, out v)) cfg.DictFileB = v;
                ...
                else if (TryOpt(args, ref i, new string[] { "--mask", "-m" }, out v)) { cfg.Mask = v; cfg.Mode = "mask"; }
                else if (TryOpt(args, ref i, new string[] { "-1" }, out v)) cfg.CustomSets[0] = v;
                else if (TryOpt(args, ref i, new string[] { "-2" }, out v)) cfg.CustomSets[1] = v;
                else if (TryOpt(args, ref i, new string[] { "-3" }, out v)) cfg.CustomSets[2] = v;
                else if (TryOpt(args, ref i, new string[] { "-4" }, out v)) cfg.CustomSets[3] = v;
                else if (TryOpt(args, ref i, new string[] { "--min" }, out v)) cfg.MaskMin = ParseInt(v, 1);
                else if (TryOpt(args, ref i, new string[] { "--max" }, out v)) cfg.MaskMax = ParseInt(v, -1);
                else if (TryOpt(args, ref i, new string[] { "-t", "--threads" }, out v)) cfg.Threads = ParseInt(v, 0);
                else if (TryOpt(args, ref i, new string[] { "--max-tries" }, out v)) cfg.MaxTries = ParseLong(v, -1);
                else if (TryOpt(args, ref i, new string[] { "--tool" }, out v)) cfg.UserTool = v;
                else if (TryOpt(args, ref i, new string[] { "--out" }, out v)) OutFile = v;
                else if (TryOpt(args, ref i, new string[] { "--extract-to" }, out v)) ExtractTo = v;
                else if (a == "--resume") Resume = true;
                else if (a == "--no-checkpoint") cfg.CheckpointEnabled = false;
            ...
            if (Resume) { cfg.ResumeRequested = true; cfg.CheckpointEnabled = true; }
            return cfg;
        }
        ...
        private static int CmdCrack(string[] args)
        {
            ...
            if (!Resume && !cfg.CheckpointEnabled) { /* nothing */ }
            if (!Resume)
            {
                // a fresh run ignores (but keeps) any stale session until the
                // first checkpoint overwrites it; a mismatched session is
                // ignored by the engine anyway
            }
            if (Resume) cfg.CheckpointEnabled = true;
        ...
        private static void PrintUsage()
        {
            ...
            Console.WriteLine("  dictcrack info <压缩包>          查看格式 / 加密方式 / 验证路径");
            ...
            Console.WriteLine("  --tool <exe>         后备验证工具路径（默认自动找 7z.exe/rar.exe）");
